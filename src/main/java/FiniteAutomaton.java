...
public class FiniteAutomaton extends FiniteStateMachine {
   ...
   /** constructs a finite automaton with the given set of states and
       start state */
   public FiniteAutomaton(State[] states, State start) {
      this(states, start, new TransitionFunction(false));
   }

   /** constructs a finite automaton with the given set of states, start state,
   ...
   /** adds a new state to this finite automaton */
   ...
   /** removes a state from this finite automaton */
   ...
   public void step() {
      synchronized (states) {
         if (answer != null) {
            if (State.DEBUG) {
               System.err.println("FiniteAutomaton.step: attempting to step " +
